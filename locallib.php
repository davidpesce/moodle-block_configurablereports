 ...
 function cr_print_table($table, $return=false) {
    ...
    if (!empty($table->head)) {
        ...
        foreach ($table->head as $key => $heading) {
            ...
            $output .= '<th style="vertical-align:top;'. $align[$key].$size[$key] .';white-space:normal;color:#F60;padding-right:20px" class="header c'.$key.$extraclass.'" scope="col">'. $heading .'</th>';
        }
        $output .= '</tr></thead>'."\n";
    }
